...
- **模型切换与视频切换**：支持 YOLO-World-V2（开放词汇，PT）/ YOLOv8-Base（COCO 80 类，PT）/ YOLOv8-Car（车辆专用，ONNX）/ YOLOv8-VisDrone（ONNX）/ YOLOv8-Person（人体专用，ONNX）/ YOLOv8-Thermal-Person（热红外人体专用，ONNX）六种模型，前四种 PT / ONNX 双运行时，后两种为 ONNX
...
- **模型评估页（评测结果 + 场景分析 + 典型案例分析���**：总览 / 训练过程 / 标准评测 / 类别分析四个 Tab，接入真实 PR 曲线图片，由独立 mock 数据驱动
- **PT / ONNX runtime 双路径**：PyTorch（YOLO-World-V2、YOLOv8-Base）与 ONNX（YOLOv8-Car、YOLOv8-VisDrone、YOLOv8-Person、YOLOv8-Thermal-Person）两条路径统一向前端暴露 timing 字段；TensorRT 为后续扩展方向
...
│   │   ├── registry.py          # 双注册表：MODEL_REGISTRY + RUNTIME_CONFIG
...
│       ├── api/                  # API 层
│       ├── composables/           # 可复用逻辑（WS / 视频流 / Canvas / 模型配置）
│       ├── layouts/MainLayout.vue # 侧边栏 + 顶栏布局
│       ├── components/            # 组件（Detection/ 任务助手等）
│       ├── data/                  # mock 数据（Performance.vue）
│       └── views/
...
├── weights/                      # 模型权重（需单独下载）
│   ├── yolov8n.pt               # YOLOv8 nano（COCO）
│   ├── yolov8s-worldv2.pt      # YOLO-World V2（推荐，开放词汇）
│   ├── yolov8_car.onnx         # 车辆检测 ONNX（VisDrone Fine-tuned）
│   ├── VisDrone/
│   │   └── yolov8x_visdrone_best.onnx # VisDrone ONNX
│   └── person_only/
│       ├── best_person.onnx         # 可见光人体检测 ONNX
│       └── best_thermal_person.onnx  # 热红外人体检测 ONNX
...
- `model_id`：本次推理使用的模型 ID（Phase 5+ 冷加载状态通知）
