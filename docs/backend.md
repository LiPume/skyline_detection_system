...
- 内部根据 `model_id` 分化：
  - `YOLO-World-V2` → `_YOLOWorldV2`：调用 `set_classes(prompt)` + `predict`
  - `YOLOv8-Base` → `_YOLOv8Base`：直接 `predict`
...
```python
{
  "YOLO-World-V2": ModelCapabilities(
    ...
    model_type="open_vocab",   # 开放词汇
    supported_classes=None,     # 无固定类别列表
    description="...",
  ...
  "YOLOv8-Base": ModelCapabilities(
    ...
    model_type="closed_set",   # 固定类别
    supported_classes=COCO_CLASSES,  # ["person","car","truck",...]
...
**RUNTIME_CONFIG**：后端执行配置

```python
{
  "YOLO-World-V2": ModelConfig(
    model_id="YOLO-World-V2",
    runtime_type="pytorch",    # PyTorch 运行时
    weight_path="weights/yolov8s-world-v2.pt",
    device="cuda:0",
    confidence=0.25,
    nms_iou=0.5,
    input_size=640
  ...
  "YOLOv8-Car": ModelConfig(
    runtime_type="onnx",       # ONNX 运行时
    weight_path="weights/yolov8-car.onnx",
    device="cuda:0",
    confidence=0.3,
    ...
  ),
...
|----------|-------------|------------|------|
| YOLO-World-V2 | pytorch | open_vocab | 开放词汇通用检测（推荐） |
| YOLOv8-Base | pytorch | closed_set | COCO 80 类基准模型 |
| YOLOv8-Car | onnx | closed_set | 车辆专项（car/bus/truck/bicycle） |
| YOLOv8-VisDrone | onnx | closed_set | VisDrone 10 类航拍数据集 |
| YOLOv8-Person | onnx | closed_set | 人员专项（person） |
| YOLOv8-Thermal-Person | onnx | closed_set | 热成像人员检测（IR图像） |

**共享类别列表**：
- `COCO_CLASSES`：80 类（COCO 数据集标准）
- `VEHICLE_CLASSES`：`['car', 'truck', 'bus', 'motorcycle', 'bicycle']`
- `VISDRONE_CLASSES`：10 类（VisDrone 标准）
...
| status | String | finished / failed / paused |
...
extra_data = {
    ...
    "detection_summary": { ... }  # 本地规则摘要
}

# generate_short_report 成功后补写
extra_data["short_report"] = "本次检测..."  # patchHistoryExtraData 合并写入
```

**to_dict() 重建逻辑**：将 `extra_data.detection_model` 展开为扁平结构，用于 API 兼容。
...
| 闭集优先 | 无特殊属性，普通目标 | YOLOv8-Base（COCO 覆盖广） |
...
2. 心跳：`receive_text == "ping"` → `send_text("pong")`
...
- `prompt_classes`：open_vocab 模型使用，逗号分隔后的列表
...
**Timing 说明**：
- `inference_time_ms` = `session_ms + preprocess_ms + postprocess_ms`
- ONNX 运行时 `preprocess_ms`/`postprocess_ms` 为 `time.perf_counter()` 计时
- PT 运行时 `session_ms` 来自 Ultralytics `Results.speed['inference']`
...
| 限制 | 说明 |
|------|------|
| 模型文件未下载 | YOLO-World-V2 PT 文件未在 weights 目录时后端报错 |
...
- 路径：`skyline/data/detections.db`（aiosqlite）
